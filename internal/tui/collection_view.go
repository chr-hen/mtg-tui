...
// showWants displays the user's wanted cards
func (a *App) showWants() {
	// Check if wants is empty
	if len(a.wants.WantedPrintings) == 0 {
		modal := tview.NewModal().
			SetText("Your wants list is empty.\n\nMark cards as wanted from the card detail view.").
			AddButtons([]string{"OK"}).
			SetDoneFunc(func(buttonIndex int, buttonLabel string) {
				a.pages.RemovePage("wants_empty")
				if a.pages.HasPage("collection_menu") {
					a.pages.SwitchToPage("collection_menu")
				} else {
					a.showCollectionMenu()
		...
		a.pages.AddPage("wants_empty", modal, true, true)
		return
	}
	...
	a.showCardsFromSource(
		...
		models.CardListConfig{
			PageName:   "wants",
			TitleFunc:  a.updateWantsTitle,
			FooterText: "f: filter | s: sort | S: direction | j/k: navigate | h/l: pages | Enter: view details | Esc: back",
			FilterFunc: a.applyWantsFilters,
			OnEscape: func() {
				if a.pages.HasPage("collection_menu") {
					a.pages.SwitchToPage("collection_menu")
				} else {
...
// updateWantsTitle updates the title of the wants view
func (a *App) updateWantsTitle() {
	pageSize := a.settings.PageSize
	// Use TotalCards from pagination info (total unique cards across all pages)
	totalUniqueCards := a.pagination.TotalCards
	totalPages := 1
	...
	title := fmt.Sprintf("My Wants - Page %d/%d", a.currentPage, totalPages)
	if a.wantsFilterQuery != "" {
		// Truncate long filter queries
		filterDisplay := a.wantsFilterQuery
		if len(filterDisplay) > 25 {
			filterDisplay = filterDisplay[:22] + "..."
		}
		title += fmt.Sprintf(" | Filter: %s", filterDisplay)
	...
	if totalUniqueCards > 0 {
		title += fmt.Sprintf(" | Total: %d unique cards", totalUniqueCards)
		// Count total printings across all wanted printings
		totalPrintings := len(a.wants.WantedPrintings)
		title += fmt.Sprintf(" (%d printings)", totalPrintings)
	} else {
		title += " | Empty"
	}
